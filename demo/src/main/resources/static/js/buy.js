...
const getGames = function (gamename) {
    fetch('/getGameItem/'+gamename, {
        ...
        let price_cachestr = numberWithCommas(price)
        let addTableContent = `
            <tr>
                <td>${text.gamename}</td>
                <td>${text.category}</td>
        ...
        let totalPrice_cachestr = numberWithCommas(totalPrice)
        let totalprice_str = `
            <p class="count_num"> 총 금액 : ${totalPrice_cachestr} 원 </p>
        `;
...
function userbuyit(){
    ...
    if(chk){
        fetch('/myitem/add', {
            ...
            }else if(text.startsWith('재고가')){
                let msgs = text.substring(5)
                alert(msgs+' 재고가 없습니다.')
            }
            else{
